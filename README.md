...
> 건조 상태에서 처리되던 연속 교차로 회랑도 우천으로 진입용량이 수요 아래로 떨어지면 짧은 연결도로의 대기행렬이 상류까지 역류할 수 있으므로, 운영자가 무대응과 복구 정책의 효과·부작용·안전 근거를 같은 조건에서 비교할 수 있어야 한다.

## 우리가 만드는 것

- 실제 세종 표준노드링크의 성금교차로 → 청사교차로 → 세종교차로 절재로 연결 관계와 성금 → 세종 대체 경로
...
화면의 지명·링크 관계는 2026-07-16 전국표준노드링크에서 세종 지역코드 413으로 추출한 8,768개 노드·11,893개 링크를 사용합니다. 단, 화면의 점유율·대기열·저장공간·개선율은 실제 세종시 성과가 아니라 동일한 합성 수요와 고정 난수 시드에서 계산된 데모용 시뮬레이션 결과입니다.
...
| [RainFlow Sejong](docs/09_RAINFLOW_SEJONG.md) | 우천 연속 교차로 회랑 디지털 트윈 설계와 검증 기준 |
