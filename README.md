...
**RainFlow Sejong**은 세종시 **절재로 회랑(성금교차로 → 청사교차로 → 세종교차로)**을 대상 구간으로, 우천으로 인한 진입용량 저하와 spillback을 합성 시나리오로 재현하고, 세 가지 대응 정책을 비교한 뒤 결정론적 안전·공정성 가드와 운영자 승인을 거치는 오프라인 디지털 트윈 프로토타입입니다.

이 프로젝트는 실제 세종 도로 또는 신호기를 제어한다고 주장하지 않습니다. **상태 재현 → 정책 비교 → 안전검사 → 규칙 기반 설명과 순위화 → 운영자 승인 → 결과 기록** 흐름을 합성 데이터로 시연합니다.

### 무엇이 확인된 값이고 무엇이 가정인가

2026-07-29 실제 세종 도로망 A안 결정에 따라 대상 구간을 실제 교차로로 지정했습니다. 확인 범위는 다음과 같습니다.

| 항목 | 상태 | 근거 |
|---|---|---|
| 교차로 이름과 위치 | **확인** | 국토교통부 표준노드링크 `[2026-07-16]` 세종 지역코드 413 추출 |
| 교차로 형식(회전 또는 신호) | 미확인 | 표준노드링크에 회전교차로 전용 노드유형 없음. 2023년 공개 신호자료 기준으로는 고정시간 신호제어 |
| 링크 길이·차로수·용량 | 모형 가정 | 세종시 회신 대기 |
| 교통 수요와 KPI 수치 | 합성(`synthetic-v0`, `provisional`) | 결정론적 큐 모델 계산 결과 |

세 교차로를 **회전교차로라고 단정하지 않습니다.** 화면의 우회경로는 공식 자료로 확인된 도로가 아니라 전환 수요를 표현하는 모형 가정 경로이며 화면에도 그렇게 표시됩니다.
...
- 절재로 회랑 3개 교차로(성금 → 청사 → 세종)와 모형 가정 우회경로
