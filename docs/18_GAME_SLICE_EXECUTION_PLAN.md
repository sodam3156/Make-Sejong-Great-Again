...
| 손시우 | 2.4절 세 방향 중 채택안 결정 | 결정 문서화 |
| 최영 | 채택안 구현 + 4장 승리 가능성 테스트 추가 | 세 미션 전부 승리 가능 정책 1개 이상 |
...
| 손시우 | 전체 진행 속도·난이도 승인 | 승급이 다음 미션을 잠그지 않음 |
...
| 각 AI를 이길 수 있는 정책이 최소 하나 | 도달 가능 상태 전수 테스트 | **불충족** |
...
1. 세 체크아웃 중 정본 하나를 확정하고 나머지를 폐기할지
2. 동결 아티팩트 재생성에 대한 준·최영 공동 승인
3. Unity 폴백 판단 시점을 8월 6일 정오로 둘지
4. 지역 선택이 연출만 바꾸고 난이도는 동일하다는 사실을 발표에서 어떻게 말할지
